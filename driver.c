...
static int pen_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
    ...
    printk(KERN_INFO "### Pen Drive Anderson (%04X:%04X) foi plugado\n", id->idVendor, id->idProduct);
    printk("Pen Drive Anderson");
...
{
        printk(KERN_INFO "[*] O Pen Drive de Anderson foi removido\n");
}
...
{
        int ret = -1;
        printk(KERN_INFO "[*] Pen Drive Anderson - Construtor do driver [*] \n");
...
{
        printk(KERN_INFO "[*] Pen Drive Anderson - Desconstrutor do driver[*]\n");
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Anderson dos Santos");
MODULE_DESCRIPTION("Principios de Software Basico");
